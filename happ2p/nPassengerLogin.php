			...
			if($action == 'generate'){				//If action is generate OTP
				...
				if($mobileNumberCount == 0 && $deviceIdCount == 0){         	//new user registration
					...
					if($deviceIdAndMobCount > 0){
						...
						$result['referredByStatus'] = 'True';
						$refSql = "(SELECT referralCode FROM userRegistration WHERE mobile='$mobileNumber')";
						$refRes = mysqli_query($con, $refSql);
						if($refR1 = mysqli_fetch_assoc($refRes)){
							$insReferralCode = $refR1['referralCode'];
						}
						$sql="INSERT INTO userRegistration(`mobile`,`fullName`,`otp`, `referralCode`, `deviceId`,`referredBy`,`tokenId`) VALUES 
										('$mobileNumber','$username','$pin', '$insReferralCode',
